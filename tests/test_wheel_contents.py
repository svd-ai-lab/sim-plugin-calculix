...
hatchling picks it up via ``packages = ["src/sim_plugin_calculix"]``
...
def test_wheel_contains_skills(tmp_path: Path) -> None:
    ...
    wheels = list(out_dir.glob("sim_plugin_calculix-*.whl"))
    ...
    required = {
        "sim_plugin_calculix/__init__.py",
        "sim_plugin_calculix/driver.py",
        "sim_plugin_calculix/_skills/calculix/SKILL.md",
    }
